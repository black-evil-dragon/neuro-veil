...
from veil.utils.numbers import parse_quotation
...
class InstrumentDataModel:
    ...
    def get_additional_candles(
        ...
        for instrument in additional_instruments:
            ...
            result = result | {instrument.get("ticker"): prepared_additional_candles}
    ...
    def get_candles(self, from_date, to_date, intervalType: str) -> dict:
        args = dict(
            from_date=from_date,
            to_date=to_date,
        ...
        log.info(
            f"[InstrumentData] Получаем котировки ({args['limit']}) по инструменту: {self.instrument.get('ticker', '')}"
        )
    ...
    def prepare_indicators(indicators):
        ...
        for indicator in indicators["technicalIndicators"]:
            processed_data[indicator["timestamp"]] = {
    ...
    def dict_to_dataframe(
        cls, data: typing.Dict[str, typing.Dict[str, typing.Any]]
    ) -> pd.DataFrame:
        """
    ...
    def differentiate_data(
        ...
        df.dropna(
            inplace=True
        )  # Удаляем NaN, которые появляются после дифференцирования
        return df
    ...
    def iterate_over_time_range(cls, step: int):
        ...
        def decorator(func):
            ...
            def wrapper(self, from_date, to_date, *args, **kwargs):
                ...
                first_candle_date = datetime.strptime(
                    first_candle_date, "%Y-%m-%dT%H:%M:%SZ"
                ).replace(tzinfo=timezone.utc)
                ...
                for start in range(0, total, step):
                    if is_hours:
                        ...
                        current_to_date = min(
                            from_date + timedelta(hours=start + step), to_date
                        )
                    else:
                        current_from_date = from_date + timedelta(days=start)
                        current_to_date = min(
                            from_date + timedelta(days=start + step), to_date
                        )

                    log.debug(
                        f"Обработка диапазона: {current_from_date} - {current_to_date}"
                    )
                    result.update(
