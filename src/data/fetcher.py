...
class DataFetcher:
    ...
    def __init__(self, TService, processor):
    ...
    def fetch_data(self, instrument, from_date, to_date):
        """
        ...
        tbank_candles = self.marketDataService.get_candles(
            from_date=from_date,
            to_date=to_date,
        ...
        tbank_RSI_AVG_14 = self.marketDataService.get_tech_analysis(
            from_date=from_date,
            to_date=to_date,
        ...
        tbank_SMA_CLOSE_14 = self.marketDataService.get_tech_analysis(
            from_date=from_date,
            to_date=to_date,
            instrumentUid=instrument.get("uid"),
            interval=self.marketDataService.IndicatorIntervalType.ONE_HOUR,
        ...
        tbank_RSI_CLOSE_14 = self.marketDataService.get_tech_analysis(
            from_date=from_date,
            to_date=to_date,
            instrumentUid=instrument.get("uid"),
            interval=self.marketDataService.IndicatorIntervalType.ONE_HOUR,
        ...
        # Обработка данных
        processed_candles = self.processor.process_candles(tbank_candles)
        for candle in processed_candles:
            data[candle["time"]] = candle

        processed_indicators = self.processor.process_indicators(tbank_RSI_AVG_14)
        for indicator in processed_indicators:
            data[indicator["time"]]["RSI_AVG_14"] = indicator["value"]

        processed_indicators = self.processor.process_indicators(tbank_RSI_CLOSE_14)
        for indicator in processed_indicators:
            data[indicator["time"]]["RSI_CLOSE_14"] = indicator["value"]

        processed_indicators = self.processor.process_indicators(tbank_SMA_CLOSE_14)
        for indicator in processed_indicators:
    ...
    def get_data(self, instrument, from_date, to_date, step=100):
        """
        ...
        for start_day in range(0, total_days, step):
            ...
            try:
                for line in self.fetch_data(instrument, current_from_date, current_to_date):
                ...
                log.error(f"Ошибка при обработке диапазона {current_from_date} - {current_to_date}: {e}")
                continue
